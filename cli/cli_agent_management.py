...
def create_agent_workflow(self):
    ...
    if next_action == "Test Agent":
        ...
        if 'synthetic_trades_path' in agent_config and os.path.exists(agent_config['synthetic_trades_path']):
            ...
            # Need access to trade_analysis_menu, assuming it exists via self
...
def edit_agent_workflow(self):
    ...
    edit_action = questionary.select(
        ...
        choices=[
            ...
            "Features",
            "Feature Parameters",
            "Generate/Update Synthetic Trades",
            "Retrain Agent",
            "Delete Agent",
            "Back to Agent Management"
        ]
...
def test_agent(self, agent_name):
    ...
    if test_result:
        ...
        try:
            ...
            initial_capital = float(agent_config.get('account_size', 10000)) # Use config or default
            fees = 0.001 # Example fee (0.1%) - make configurable?
            sl_pct = agent_config.get('stop_loss_pct', None) # Get SL from config if exists
            tp_pct = agent_config.get('take_profit_pct', None) # Get TP from config if exists
